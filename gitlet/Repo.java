 ...
 * the Repo is the overseer of the entire .gitlet repository.
 */
public class Repo implements Serializable {

    /*********************** INIT ****************************/

    /** Creates a new Gitlet version-control system in the current directory. This system will automatically start
     * with one commit: a commit that contains no files. It will have a single branch: master, which initially points to this initial
     ...
     * tree until the initial commit, following the first parent commit links, ignoring any second parents found in merge commits.
     * This set of commit nodes is called the commit's history. For every node in this history, the information it should display
    ...
    /** Takes in a UID for a commit, and prints out the commit, what it prints out depends on whether it is a merge
    ...
    /** Exactly like the regular commit function, but used for merge commits, takes in a String MSG, and
     * a set of PARENTS. */
    ...
    /** Unstage the file if it is currently staged. If the file is tracked in the current commit, mark it to indicate that it is
     * not to be included in the next commit (presumably you would store
     * this mark somewhere in the .gitlet directory), and remove the file from the working directory if the user has not already done so
    ...
    /** This function takes in the present working directory PWD and will determine if there are untracked files
    ...
    /**
     * Checks out all the files tracked by the given commit.it removes tracked files that are not present in that commit.
     * Also moves the current branch's head to that commit node. See the intro for an example of what happens to the head pointer after using reset. The [commit id] may be abbreviated as for checkout.
    ...
    /**
     * Failure case not yet addressed: If an untracked file in the current commit would be overwritten or delted by the merge,
     * print There is an untracked file in the way; delete it or add it first. and exit; perform this check before doing anything else.
    ...
    /** This is how we are going to be capable of returns back and forth in between each hash and the corresponding commit. Takes in a
